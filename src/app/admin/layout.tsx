...
export default function AdminLayout({
  ...
  useEffect(() => {
    if (!isLoading && user) {
      // Check if user is AdminClinica
      // AdminClinica role should be "admin" with role_name "AdminClinica" or role_id pointing to AdminClinica
      const isAdminClinica = 
        user.role === 'admin' && 
        (user.role_id === 2 || // AdminClinica role_id from seed data
         user.role_name === 'AdminClinica');
      
      if (!isAdminClinica) {
        toast.error("Acesso negado. Apenas Administradores de Clínica podem acessar esta área.");
      ...
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-50 to-cyan-50">
        <div className="text-center max-w-md p-8 bg-white rounded-lg shadow-lg border border-blue-200">
          ...
          <p className="text-gray-600 mb-6">
            Você não tem permissão para acessar a área de Administração da Clínica.
            Apenas Administradores de Clínica podem acessar esta seção.
          </p>
          <button
